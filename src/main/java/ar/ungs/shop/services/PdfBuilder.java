...
public class PdfBuilder {
	...
	public PdfBuilder(String path) {
		...
		this.document.setMargins(THERMAL_MARGIN_TOP_BOTTOM, THERMAL_MARGIN_RIGHT, THERMAL_MARGIN_TOP_BOTTOM,
				THERMAL_MARGIN_LEFT);
	...
	private void prepareDocument(PageSize pageSize) {
		...
		if (!file.exists()) {
			try {
				...
				LogManager.getLogger(this.getClass())
						.error(String.format(
								"PdfBuilder::prepareDocument. Error when creating the pdf document (%s). %s",
								this.filename, s));
		...
		try {
			...
			LogManager.getLogger(this.getClass()).error(String.format(
					"PdfBuilder::prepareDocument. Error when creating the pdf document (%s). %s", this.filename, fnfe));
			throw new PdfException("Can’t create PDF (" + this.filename + ")");
		}
	}
	...
	public PdfBuilder image(String fileName) {
		try {
			...
			LogManager.getLogger(this.getClass()).error(
					String.format("PdfTicketBuilder::addImage. Error when add image to PDF (%s). %s", fileName, e));
	...
	public byte[] build() {
		...
		try {
			...
			LogManager.getLogger(this.getClass())
					.error(String.format("PdfTicketBuilder::build. Error when read bytes to PDF. %s", ioe));
